...
public class NavMesh {
    ...
    /// 指示实体链接到外部实体的标志(多边形边是连接到另一个多边形的入口)
    ...
    public static long encodePolyId(int tileSalt, int tileIndex, int indexOfPoly) {
        return (((long) tileSalt) << (DT_POLY_BITS + DT_TILE_BITS)) | ((long) tileIndex << DT_POLY_BITS) | indexOfPoly;
    }

    public static int[] decodePolyId(long ref) {
        return new int[]{decodePolyIdSalt(ref), decodePolyIdTile(ref), decodePolyIdPoly(ref)};
    }

    public static int decodePolyIdSalt(long ref) {
    ...
    public Result<Tupple2<MeshTile, Poly>> getTileAndPolyByRef(long ref) {
        if (ref == 0) return Result.invalidParam("ref = 0");
        int[] saltitip = decodePolyId(ref);
        int tileSalt = saltitip[0];
        int tileIndex = saltitip[1];
        int indexOfPoly = saltitip[2];
        if (tileIndex >= m_maxTiles)
            return Result.invalidParam("tile > m_maxTiles");
        if (m_tiles[tileIndex].salt != tileSalt || m_tiles[tileIndex].data.header == null)
            return Result.invalidParam("Invalid salt or header");
        if (indexOfPoly >= m_tiles[tileIndex].data.header.polyCount)
            return Result.invalidParam("poly > polyCount");
        return Result.success(new Tupple2<>(m_tiles[tileIndex], m_tiles[tileIndex].data.polys[indexOfPoly]));
    }
    ...
    boolean isValidPolyRef(long ref) {
        ...
        if (tileIndex >= m_maxTiles)
            return false;
        if (m_tiles[tileIndex].salt != tileSalt || m_tiles[tileIndex].data == null)
            return false;
        if (indexOfPoly >= m_tiles[tileIndex].data.header.polyCount)
            return false;
        return true;
